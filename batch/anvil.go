...
func (s *Anvil) notation(key string, rootKey string, v reflect.Value, title bool) (items []Item, err error) {
	var (
		value interface{}
		empty = true
	...
	case reflect.Array:
		...
		for i := 0; i < v.Len(); i++ {
			n, err := s.notation(arrayPrefix(key, i), rootKey, v.Index(i), true)
			if err != nil {
				return nil, err
			}
			if len(n) < 1 {
	...
	case reflect.Slice:
		...
		if reflect.TypeOf(v.Interface()).String() == "[]string" {
			itemsString := make([]string, 0)
			for i := 0; i < v.Len(); i++ {
				items_, err := s.notation(arrayPrefix(key, i), rootKey, v.Index(i), true)
				if err != nil {
					return nil, err
				}
				if len(items_) < 1 {
				...
				for i := 0; i < len(items_); i++ {
					value := items_[i]
					itemsString = append(itemsString, value.Value.(string))
				}
			}
			valueArray := Item{Key: key, Value: itemsString}
			items = append(items, valueArray)
			return items, nil
		}
	case reflect.Struct:
		if value, empty, err = s.modify(v); err != nil {
		...
		for i := 0; i < l; i++ {
			...
			n, err := s.notation(s.key(key, rootKey, v.Type().Field(i), false), rootKey, f, true)
			if err != nil {
				return nil, err
			}
			if len(n) < 1 {
	...
	case reflect.Interface:
		...
		n, err := s.notation(key, rootKey, v.Elem(), true)
		if err != nil {
			return nil, err
		}
		if len(n) < 1 {
	...
	case reflect.Map:
		...
		keys := v.MapKeys()
		for i := range keys {
			n, err := s.notation(mapPrefix(key, keys[i]), rootKey, v.MapIndex(keys[i]), true)
			if err != nil {
				return nil, err
			}
			if len(n) < 1 {
...
// key of field
func (s *Anvil) key(pref string, rootKey string, v reflect.StructField, omit bool) string {
	if omit {
		return pref
	}
	var title string
	json, ok := v.Tag.Lookup("bson")
	if !ok || len(json) < 1 {
		title = v.Name
	} else {
	...
	return strings.Replace(pref+s.Glue+title, rootKey+".", "", 1)
}
