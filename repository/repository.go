...
func NewRepository[T any](client *firestore.Client, collectionName string, options ...string) *Repository[T] {
	...
	adapter := &Repository[T]{Collection: client.Collection(collectionName), ModelType: modelType, idIndex: idx, jsonIdName: jsonIdName, Map: maps, createdTimeIndex: ctIdx, updatedTimeIndex: utIdx, versionIndex: versionIndex}
...
func (s *Repository[T]) All(ctx context.Context) ([]T, error) {
...
func (s *Repository[T]) Load(ctx context.Context, id string) (*T, error) {
	var obj T
...
func (s *Repository[T]) Exist(ctx context.Context, id string) (bool, error) {
	return f.Exist(ctx, s.Collection, id)
}
func (s *Repository[T]) Create(ctx context.Context, model *T) (int64, error) {
...
func (s *Repository[T]) Update(ctx context.Context, model *T) (int64, error) {
	mv := reflect.ValueOf(model)
	id := reflect.Indirect(mv).Field(s.idIndex).Interface().(string)
	if s.versionIndex >= 0 {
...
func (s *Repository[T]) Patch(ctx context.Context, data map[string]interface{}) (int64, error) {
...
func (s *Repository[T]) Save(ctx context.Context, model interface{}) (int64, error) {
	mv := reflect.ValueOf(model)
	id := reflect.Indirect(mv).Field(s.idIndex).Interface().(string)
	if s.versionIndex >= 0 {
...
func (s *Repository[T]) Delete(ctx context.Context, id string) (int64, error) {
